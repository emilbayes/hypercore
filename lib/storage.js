...
function Storage (create, cacheSize) {
  ...
  this.signatureAlg = 'Ed25519'
  this.secretKeyBytes = 64
  this.publicKeyBytes = 32
  this.signatureBytes = 64

  this.hashAlg = 'BLAKE2b'
  this.merkleBytes = 40
  this.hashBytes = 32
}
...
Storage.prototype._getSignature = function (index, cb) {
  this.signatures.read(HEADER_BYTES + this.signatureBytes * index, this.signatureBytes, cb)
}

Storage.prototype.putSignature = function (index, signature, cb) {
  this.signatures.write(HEADER_BYTES + this.signatureBytes * index, signature, cb)
}
...
Storage.prototype.getDataBatch = function (start, n, cb) {
  ...
  this.dataOffset(start, noarr, function (err, offset, size) {
    ...
    self.tree.read(HEADER_BYTES + 2 * self.merkleBytes * start, 2 * self.merkleBytes * n - self.merkleBytes, ontree)
    ...
    function ontree (err, buf) {
      ...
      if (buf) {
        for (var i = 1; i < sizes.length; i++) {
          sizes[i] = uint64be.decode(buf, HEADER_BYTES + (i - 1) * 2 * self.merkleBytes)
          total += sizes[i]
        }
      }
...
Storage.prototype.getNode = function (index, cb) {
  ...
  this.tree.read(HEADER_BYTES + self.merkleBytes * index, self.merkleBytes, function (err, buf) {
    if (err) return cb(err)

    var hash = buf.slice(0, self.hashBytes)
    var size = uint64be.decode(buf, self.hashBytes)
...
Storage.prototype.putNode = function (index, node, cb) {
  ...
  var buf = bufferAlloc(this.merkleBytes)

  node.hash.copy(buf, 0)
  uint64be.encode(node.size, buf, this.hashBytes)
  this.tree.write(HEADER_BYTES + this.merkleBytes * index, buf, cb)
}

Storage.prototype.putBitfield = function (offset, data, cb) {
  this.bitfield.write(HEADER_BYTES + offset, data, cb)
}
...
Storage.prototype.openKey = function (opts, cb) {
  ...
  this.key.read(0, this.publicKeyBytes, cb)
}
...
Storage.prototype.open = function (opts, cb) {
  ...
  this.bitfield.read(0, HEADER_BYTES, function (_, h) {
    if (h) result.bitfieldPageSize = h.readUInt16BE(5)
    self.bitfield.write(0, header(0, result.bitfieldPageSize, null), function (err) {
      if (err) return cb(err)
      readAll(self.bitfield, HEADER_BYTES, result.bitfieldPageSize, function (err, pages) {
  ...
  this.signatures.write(0, header(1, this.signatureBytes, this.signatureAlg), done)
  this.tree.write(0, header(2, this.merkleBytes, this.hashAlg), done)

  // TODO: Improve the error handling here.
  // I.e. if secretKey length === 64 and it fails, error

  this.secretKey.read(0, this.secretKeyBytes, function (_, data) {
    if (data) result.secretKey = data
    done(null)
  })

  this.key.read(0, this.publicKeyBytes, function (_, data) {
...
function header (type, size, name) {
  var buf = bufferAlloc(HEADER_BYTES)
  buf.fill(0)
